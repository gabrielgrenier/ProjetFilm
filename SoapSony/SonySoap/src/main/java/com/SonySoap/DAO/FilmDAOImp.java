...
public class FilmDAOImp implements FilmDAO {
    ...
    public List<Film> findAll() {
        ...
        try {
            ResultSet result = this.connect.createStatement(
                    ...
                    ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT * FROM film_sony");
    ...
    public Film findById(int id) {
        ...
        try {
            ResultSet result = this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT * FROM film_sony WHERE id = " + id);
            if(result.first()){
    ...
    public boolean delete(int id) {
        try {
            ...
            stat.executeUpdate("delete from film_sony where id=" + id);
        }catch (Exception e){
            e.printStackTrace();
            return false;
        }
    ...
    public boolean add(String titre, String date, String description, double prix, String lienImage) {
        try{
            ...
            Statement stat = connect.createStatement();
            stat.executeUpdate("insert into film_sony(titre, date_sortie, prix_visionnement, lien_image, description)" +
    ...
    public boolean edit(int id, String titre, String date, String description, double prix, String lienImage) {
        try{
            //Juste pour tester le format et faire erreur si pas bon format
            Date dateTemp =new SimpleDateFormat("yyyy-MM-dd").parse(date);
            String sql = "UPDATE `film_sony` SET " +
